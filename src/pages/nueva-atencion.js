  ...
  return `<section class="attention-page" data-role="${profile.rol}">
    ...
    <form id="attention-form" class="d-none" novalidate>
      ...
      <section class="form-section"><h3>B. Persona atendida</h3><div class="row g-3">
        <div class="col-md-6"><label class="form-label" for="tipo_persona">Tipo de persona *</label><select class="form-select" id="tipo_persona" name="tipo_persona"><option value="">Seleccionar</option><option value="estudiante">Estudiante</option><option value="docente">Docente</option><option value="administrativo">Administrativo</option><option value="visitante">Visitante</option><option value="operativo">Operativo</option></select><div class="invalid-feedback" data-error-for="tipo_persona"></div></div>
...
export const initNuevaAtencionPage = async ({ session }) => {
  ...
  const loadInventory = async () => {
    ...
    if (!sedeId) return
    try { inventory = await getInventarioDisponiblePorSede(sedeId); $('inventory-status').textContent = inventory.length ? `${inventory.length} producto(s) disponible(s) para registro.` : 'No hay productos disponibles en esta sede.'; $('add-product').disabled = !inventory.length }
  ...
  $('product-list').addEventListener('change', async (event) => {
    ...
    if (event.target.matches('.product-select')) {
      ...
      const sinDescuento = item ? !item.es_consumible || item.permite_registro_sin_descuento : false
      Object.assign(row, { producto_id: event.target.value, lote_id: null, existencia_actual: item?.existencia_actual, sinDescuento, lotes: [], loadingLotes: !sinDescuento, hasLotes: false }); renderProducts()
      if (!sinDescuento) try { row.lotes = await getLotesDisponibles(row.producto_id, $('sede_id').value); row.hasLotes = row.lotes.length > 0 } catch (error) { row.lotes = []; await Swal.fire({ icon: 'error', title: 'No fue posible consultar lotes', text: friendlyError(error) }) }
